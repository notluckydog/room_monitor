...
from room_monitor.settings import REGEX_MOBILE
...
class UserRegSerializer(serializers.ModelSerializer):
    ...
    name = serializers.CharField(label="用户名", help_text="用户名", required=True, allow_blank=False,
                                     validators=[UniqueValidator(queryset=MyUser.objects.all(), message="用户已经存在")])
    #输入密码的时候不显示明文
    ...
    def validate_code(self, code):
        ...
        verify_records = VerifyCode.objects.filter(mobile=self.initial_data["name"]).order_by("-add_time")
        #verify_records = VerifyCode.objects.filter(mobile=self.request.GET.get("name")).order_by("-add_time")
    ...
    class Meta:
        model = MyUser
        fields = ('name','code','mobile','password')